...
# conda install sklearn  # pip install scikit-learn
...
corpus = pd.concat([
    corpus_x1, corpus_x2])
# 生成词汇表（word_index）（词典）
# fit_on_texts：根据一系列文本生成token词典，texts为list类，每个元素为一个文本（此处为一个句子）
...
print(x1_train[:1]) # 第一条新闻标题
print(x1_train[0]) # 第一条新闻标题
